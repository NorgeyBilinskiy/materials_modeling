...
class MaterialsDataManager:
    ...
    def __init__(self):
        ...
        if not self.api_token:
            raise ValueError("Materials Project API token not configured. Check configuration.")
    ...
    def get_materials_data(self) -> Dict[str, pd.DataFrame]:
        ...
        logger.info(f"Retrieving data for {len(self.materials_list)} materials from Materials Project...")
        ...
        try:
            for material in self.materials_list:
                ...
                try:
                    material_data = self.client.get_compound_data(material, return_format="dataframe")
                    
                    if not material_data.empty:
                        materials_data[material] = material_data
                        logger.info(f"Successfully retrieved {len(material_data)} records for {material}")
                        
                        logger.info(f"Data columns for {material}: {list(material_data.columns)}")
                        logger.info(f"Data size for {material}: {material_data.shape}")
                    else:
                        logger.warning(f"No data found for {material} in Materials Project")
            ...
            logger.info(f"Data retrieval completed. Successfully retrieved data for {len([k for k, v in materials_data.items() if not v.empty])} materials")
    ...
    def get_materials_data_with_save(self, save_dir: str = None) -> Dict[str, pd.DataFrame]:
        """
